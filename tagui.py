...
def _tagui_output():
    ...
    init_directory_output_file = os.path.join(_tagui_init_directory,
                                              'rpa_python.txt')
...
def _tagui_delta(base_directory=None):
    ...
    if platform.system() in ['Linux', 'Darwin']:
        os.system('chmod -R 755 ' + base_directory + '/' + 'src' + '/' +
                  'tagui > /dev/null 2>&1')
        os.system('chmod -R 755 ' + base_directory + '/' + 'src' + '/' +
                  'end_processes > /dev/null 2>&1')

    # create marker file to skip syncing delta files next time for current release
    delta_done_file = _py23_open(
        base_directory + '/' + 'rpa_python_' + __version__, 'w')
    delta_done_file.write(
        _py23_write('TagUI installation files used by RPA for Python'))
...
def _patch_macos_pjs():
    ...
    if platform.system() == 'Darwin' and not os.path.isdir(
            tagui_location() + '/.tagui/src/phantomjs_old'):
        original_directory = os.getcwd()
        os.chdir(tagui_location() + '/.tagui/src')
        print(
            '[RPA][INFO] - downloading latest PhantomJS to fix OpenSSL issue')
        download(
            'https://github.com/tebelorg/Tump/releases/download/v1.0.0/phantomjs-2.1.1-macosx.zip',
            'phantomjs.zip')
...
def setup():
    ...
    if platform.system() == 'Darwin' and _python3_env():
        if os.system(
                '/Applications/Python\ 3.9/Install\ Certificates.command > /dev/null 2>&1'
        ) != 0:
            if os.system(
                    '/Applications/Python\ 3.8/Install\ Certificates.command > /dev/null 2>&1'
            ) != 0:
                if os.system(
                        '/Applications/Python\ 3.7/Install\ Certificates.command > /dev/null 2>&1'
                ) != 0:
                    os.system(
                        '/Applications/Python\ 3.6/Install\ Certificates.command > /dev/null 2>&1'
                    )
    ...
    if platform.system() == 'Linux': tagui_zip_file = 'TagUI_Linux.zip'
        ...
        print('[RPA][ERROR] - unknown ' + platform.system() +
              ' operating system to setup TagUI')
        return False
    ...
    if not os.path.isfile('rpa_python.zip'):
        ...
        if not os.path.isfile(home_directory + '/' + 'tagui' + '/' + 'src' +
                              '/' + 'tagui'):
    ...
    if platform.system() == 'Linux':
        ...
        if os.system('chmod -R 755 ' + tagui_directory +
                     ' > /dev/null 2>&1') != 0:
        ...
        if os.system('php --version > /dev/null 2>&1') != 0:
            ...
            print(
                '[RPA][INFO] - google how to install PHP (eg for Ubuntu, apt-get install php)'
            )
            print(
    ...
    if platform.system() == 'Windows':
        ...
        if os.system(tagui_directory + '/' + 'src' + '/' +
                     'php/php.exe -v > nul 2>&1') != 0:
            print(
            ...
            if not os.path.isfile(tagui_directory + '/vcredist_x86.exe'):
                ...
                if not download(vcredist_x86_url,
                                tagui_directory + '/vcredist_x86.exe'):
                    return False
            ...
            if os.system(tagui_directory + '/' + 'src' + '/' +
                ...
                print('[RPA][INFO] - the vcredist_x86.exe file in ' +
                      home_directory + '\\tagui or from')
                print(
...
def init(visual_automation=False, chrome_browser=True, headless_mode=False):
    ...
    if visual_automation:
        ...
        if os.system('java -version ' + shell_silencer) != 0:
            ...
            if '64 bit' not in java_version_info and '64-bit' not in java_version_info:
                ...
                if os.path.isfile(sikulix_folder + '/' +
                                  'jython-standalone-2.7.1.jar'):
                    os.system('java -jar ' + sikulix_folder + '/' +
                              'sikulix.jar -h ' + shell_silencer)
                _visual_flow()
    else:
        _python_flow()
    ...
    try:
        ...
        _process = subprocess.Popen(tagui_cmd,
                                    shell=True,
        ...
        while True:
            ...
            if 'LIVE MODE - type done to quit' in tagui_out:
                ...
                _tagui_write('echo "[RPA][' + str(_tagui_id) +
...
def pack():
    ...
    print(
        '[RPA][INFO] - pack() is to deploy RPA for Python to a computer without internet'
    )
    print(
        '[RPA][INFO] - update() is to update an existing installation deployed from pack()'
    )
    print(
        '[RPA][INFO] - detecting and zipping your TagUI installation to rpa_python.zip ...'
    )
    ...
    if not download(sikulix_jython_url,
                    sikulix_directory + '/' + 'jython-standalone-2.7.1.jar'):
        return False
...
def update():
    ...
    v_back_marker = "'"
    rpa_python_py = rpa_python_py[rpa_python_py.find(v_front_marker) +
                                  len(v_front_marker):]
...
"""
    ...
    try:
        ...
        zip_base64_data = (base64.b64encode(
            update_zip_file.read())).decode('utf-8')
        update_zip_file.close()
        ...
        print(
            '[RPA][INFO] - done. copy or email update.py to your target computer and run'
        )
        print(
            '[RPA][INFO] - python update.py to update RPA for Python to version '
            + rpa_python_py)
        print(
            '[RPA][INFO] - to use custom TagUI folder, set base_directory in update.py'
        )
        return True
...
def _ready():
    ...
    try:
        ...
        if tagui_out.strip().startswith('[RPA][') and tagui_out.strip(
        ).endswith('] - listening for inputs'):
            return True
        else:
            return False
...
def close():
    ...
    try:
        ...
        if os.path.isfile(os.path.join(_tagui_init_directory,
                                       'rpa_python.js')):
        ...
        if not debug():
            ...
            # to handle user changing current directory after init() is called
            if os.path.isfile(
                    os.path.join(_tagui_init_directory, 'rpa_python.log')):
                os.remove(os.path.join(_tagui_init_directory,
                                       'rpa_python.log'))
            if os.path.isfile(
                    os.path.join(_tagui_init_directory, 'rpa_python.txt')):
                os.remove(os.path.join(_tagui_init_directory,
                                       'rpa_python.txt'))
...
def exist(element_identifier=None):
    ...
    if element_identifier.lower() in ['page.png', 'page.bmp']:
        if _visual():
            ...
            print(
                '[RPA][ERROR] - page.png / page.bmp requires init(visual_automation = True)'
            )
            return False

    # pre-emptive checks if image files are specified for visual automation
    if element_identifier.lower().endswith(
            '.png') or element_identifier.lower().endswith('.bmp'):
        if not _visual():
            print('[RPA][ERROR] - ' + element_identifier +
                  ' identifier requires init(visual_automation = True)')
            return False
        ...
        if len(element_identifier.split(',')) in [2, 3]:
            if not any(c.isalpha() for c in element_identifier):
                if _visual():
                    ...
                    print(
                        '[RPA][ERROR] - x, y coordinates require init(visual_automation = True)'
                    )
                    return False
...
def type(element_identifier=None, text_to_type=None, test_coordinate=None):
    ...
    elif not send('type ' + _sdq(element_identifier) + ' as ' +
                  _esq(text_to_type)):
        return False
...
def select(element_identifier=None,
           option_value=None,
           test_coordinate1=None,
           test_coordinate2=None):
    if not _started():
    ...
    if element_identifier.lower() in [
            'page.png', 'page.bmp'
    ] or option_value.lower() in ['page.png', 'page.bmp']:
        print(
            '[RPA][ERROR] - page.png / page.bmp identifiers invalid for select()'
        )
        return False
    ...
    if option_value.lower().endswith('.png') or option_value.lower().endswith(
            '.bmp'):
        if not _visual():
            print('[RPA][ERROR] - ' + option_value +
                  ' identifier requires init(visual_automation = True)')
            return False
    ...
    elif not send('select ' + _sdq(element_identifier) + ' as ' +
                  _esq(option_value)):
        return False
...
def read(element_identifier=None,
    ...
    if test_coordinate1 is not None and isinstance(test_coordinate1, int):
        if test_coordinate2 is not None and isinstance(test_coordinate2, int):
            if test_coordinate3 is not None and isinstance(
                    test_coordinate3, int):
                element_identifier = coord(element_identifier,
                                           test_coordinate1) + '-'
                element_identifier = element_identifier + coord(
                    test_coordinate2, test_coordinate3)
...
def snap(element_identifier=None,
    ...
    if isinstance(element_identifier, int) and isinstance(
            filename_to_save, int):
    ...
    elif not send('snap ' + _sdq(element_identifier) + ' to ' +
                  _esq(filename_to_save)):
        return False
...
def keyboard(keys_and_modifiers=None):
    ...
    if not _visual():
        print(
            '[RPA][ERROR] - keyboard() requires init(visual_automation = True)'
        )
        return False
...
def table(element_identifier=None, filename_to_save=None):
    ...
    elif not send('table ' + _sdq(element_identifier) + ' to ' +
                  _esq(filename_to_save)):
        return False
...
def upload(element_identifier=None, filename_to_upload=None):
    ...
    elif not send('upload ' + _sdq(element_identifier) + ' as ' +
                  _esq(filename_to_upload)):
        return False
...
def frame(main_frame=None, sub_frame=None):
    ...
    send(
        'js chrome_step("Runtime.evaluate", {expression: "mainframe_context = null"})'
    )
    send(
        'js chrome_step("Runtime.evaluate", {expression: "subframe_context = null"})'
    )
    send(
        'js chrome_context = "document"; frame_step_offset_x = 0; frame_step_offset_y = 0;'
    )
    ...
    if not exist(frame_identifier):
        print('[RPA][ERROR] - cannot find frame with @name or @id as \'' +
              main_frame + '\'')
        return False

    send('js new_context = "mainframe_context"')
    send('js frame_xpath = \'(//frame|//iframe)[@name="' + main_frame +
         '" or @id="' + main_frame + '"]\'')
    send('js frame_rect = chrome.getRect(xps666(frame_xpath))')
    send(
        'js frame_step_offset_x = frame_rect.left; frame_step_offset_y = frame_rect.top;'
    )
    send(
    ...
    if sub_frame is not None and sub_frame != '':
        ...
        if not exist(frame_identifier):
            print(
                '[RPA][ERROR] - cannot find sub frame with @name or @id as \''
                + sub_frame + '\'')
            return False

        send('js new_context = "subframe_context"')
        send('js frame_xpath = \'(//frame|//iframe)[@name="' + sub_frame +
             '" or @id="' + sub_frame + '"]\'')
        send('js frame_rect = chrome.getRect(xps666(frame_xpath))')
        send(
            'js frame_step_offset_x = frame_rect.left; frame_step_offset_y = frame_rect.top;'
...
def popup(string_in_url=None):
    ...
    send(
        'js found_targetid = ""; chrome_targets = []; ws_message = chrome_step("Target.getTargets", {});'
    )
    send(
    ...
    send('js chrome_targets.forEach(function(target) {if (target.url.indexOf("'
         + string_in_url + '") !== -1) found_targetid = target.targetId;})')
    send(
    ...
    if popup_result != '':
        ...
        print('[RPA][ERROR] - cannot find popup tab containing URL string \'' +
              string_in_url + '\'')
        return False
...
def api(url_to_query=None):
    print(
        '[RPA][INFO] - although TagUI supports calling APIs with headers and body,'
    )
    print(
        '[RPA][INFO] - recommend using requests package with lots of online docs'
    )
    return True
...
def run(command_to_run=None):
    if command_to_run is None or command_to_run == '':
        ...
        if platform.system() == 'Windows':
            ...
            command_delimiter = '; '
        return _py23_decode(
            subprocess.check_output(command_to_run + command_delimiter +
                                    'exit 0',
...
def vision(command_to_run=None):
    ...
    if not _visual():
        print(
            '[RPA][ERROR] - vision() requires init(visual_automation = True)')
        return False
...
def present(element_identifier=None):
    ...
    send('present_result = present(\'' + _sdq(element_identifier) +
         '\').toString()')
...
def count(element_identifier=None):
    ...
    send('count_result = count(\'' + _sdq(element_identifier) +
         '\').toString()')
...
def mouse_xy():
    ...
    if not _visual():
        print(
            '[RPA][ERROR] - mouse_xy() requires init(visual_automation = True)'
        )
        return ''
...
def mouse_x():
    ...
    if not _visual():
        print(
            '[RPA][ERROR] - mouse_x() requires init(visual_automation = True)')
        return int(0)
...
def mouse_y():
    ...
    if not _visual():
        print(
            '[RPA][ERROR] - mouse_y() requires init(visual_automation = True)')
        return int(0)
...
def clipboard(text_to_put=None):
    ...
    if not _visual():
        print(
            '[RPA][ERROR] - clipboard() requires init(visual_automation = True)'
        )
        return False
    ...
    elif not send("js clipboard('" +
                  text_to_put.replace("'", '[BACKSLASH_QUOTE]') + "')"):
        return False
